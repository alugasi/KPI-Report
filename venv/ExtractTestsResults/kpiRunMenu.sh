...
PATH_TO_JSON_FOLDERS="$(pwd)/JsonTestsFolders"
    ...
    # Display the menu and prompt the user for input
        ...
        do
            ...
            "PTP")
                ...
                source ./CreateNetwork/extractResultsFromNetwork.sh 0 $path "$config_PTP"
                break
                ;;
            "RFC")
                ...
                source ./CreateNetwork/extractResultsFromNetwork.sh 1 $path $nic_RFC
                break
                ;;
            "CYCLICTEST")
                ...
                source ./CreateAvailability/extractResultsFromAvailability.sh 1 $path $operator_version
                break
                ;;
            "OSLAT")
                ...
                source ./CreateAvailability/extractResultsFromAvailability.sh 0 $path $operator_version
                break
                ;;
            "CPU UTIL")
                echo "You selected CPU UTIL"
                read -p "enter CPU UTIL test output path: " path
                source ./CreateCpuUtil/extractResultsFromCPUUtil.sh $path
                break
                ;;
            "REBOOT")
                ...
                source ./CreateReboot/extractResultsFromReboot.sh $path $operator_version
                break
                ;;
            "DEPLYOMENT")
                echo "You selected DEPLYOMENT"
                read -p "enter DEPLYOMENT test output path: " path
                source ./CreateDeployment/ExtractResultsFromDeployment.sh $path
                break
                ;;
            "QUIT")
                echo "Exiting"
